...
"""
Created on Tue Sep 18, 2023
- This script contains modified code for Transformer model from PyTorch library.
- This allows to handle `NaN` properly while grdaient backpropagation.
- Tries to handle `NaN` in the following way:
    - mask the irrelevant positions in `Q*K^T` to `-inf` before `softmax()` as suggested by `attn_mask`.
    - Set the 'NaN' coming out after `softmax()` of `Q*K^T` to `0.0`.
"""
